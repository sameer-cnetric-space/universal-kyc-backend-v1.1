...
const { compareFacesAndLiveliness } = require("./faceRecog");
...
const runModerationChecks = async (
  ...
  try {
    ...
    faceRecogData.aadhaarSelfie = await handleAPICall(
      "Face Comparison",
      () => compareFacesAndLiveliness(selfiePath, aadhaarImage),
      errorLogs
    );
    ...
    faceRecogData.idSelfie = await handleAPICall(
      "Face Comparison",
      () => compareFacesAndLiveliness(selfiePath, frontDocumentPath),
      errorLogs
    );
    ...
    faceRecogData.pptPhotoSelfie = await handleAPICall(
      "Face Comparison",
      () => compareFacesAndLiveliness(selfiePath, pptPhoto),
      errorLogs
    );
    ...
    //   "Face Liveness check",
    //   () => compareFacesAndLiveliness(selfiePath),
    //   errorLogs
    // );
